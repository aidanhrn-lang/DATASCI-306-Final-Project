...
This project is a webapp built using Shiny in order to measure and analyze the predictability of NFL teams' scores under varying conditions. The webapp allows users to adjust a number of variables to analyze the predictability of teams. The varaibles that can be adjusted include: wind speed, home rest days, away rest days, surface type of the field, indoor/outdoor status of the field, the time of day the games are played, and whether or not games are played on a weekend. The user can also selesct the range of years they wish to pull data from to analyze. 
